  ...
  <div class="token-generator">
    <a-form layout="vertical" class="token-form" :model="form">
      <a-form-item label="AppKey" required>
        <a-input
          v-model="form.appkey"
          placeholder="请输入AppKey (格式: xxx#xxx)"
        />
      </a-form-item>
...
const generateToken = () => {
  ...
  try {
    ...
    // f. 进行base64编码（URL安全）
    let tokenBase64 = CryptoJS.enc.Base64.stringify(
      CryptoJS.enc.Utf8.parse(tokenWithPrefix),
    );
    // 转换为URL安全的Base64
...
const verifyToken = () => {
  ...
  try {
    ...
    verifyResult.value = {
      isValid: false,
      errorMessage:
        error instanceof Error ? error.message : 'Token解析失败，请检查格式',
    };
