  ...
  <div class="tool-content im-network-test-generator">
    ...
    <a-card class="generator-card">
      <a-form
        layout="horizontal"
        label-align="right"
        :label-col="{ xs: 8, sm: 6, md: 5 }"
        :wrapper-col="{ xs: 16, sm: 18, md: 19 }"
        :model="formData"
      >
...
const generateLink = () => {
  ...
  // 根据服务器类型选择不同的完整域名
  const baseUrl =
    formData.value.serverType === 'domestic'
      ? 'https://downloads.easemob.com/downloads/WEB_SDK/tools/index.html'
      : 'https://tools.easemob.com/';

  // 生成链接
  generatedLink.value = `${baseUrl}?username=${encodeURIComponent(
    formData.value.userId,
  )}&accessToken=${encodeURIComponent(
    formData.value.token,
  )}&appKey=${encodeURIComponent(
    formData.value.appKey,
  )}&serverType=${encodeURIComponent(formData.value.serverType)}`;
